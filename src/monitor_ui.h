...
enum MonitorPage {
  MONITOR_PAGE_CPU_RAM,
  MONITOR_PAGE_DISK_NET,
  MONITOR_PAGE_GPU_SYS,
...
class MonitorUi {
 ...
 private:
  ...
  void drawCpuRamData(const PcStats& stats);
  void drawDiskNetData(const PcStats& stats);
  void drawGpuSysData(const PcStats& stats);
  ...
  void pushCpuHistory(float cpuUsage);
  ...
  MonitorPage page_ = MONITOR_PAGE_CPU_RAM;
  MonitorPage chromePage_ = MONITOR_PAGE_CPU_RAM;
  bool chromeDrawn_ = false;
  bool dirty_ = true;

  static const int kHistoryLen = 32;
  uint8_t cpuHistory_[32] = {};
